...
#Extra Plots for social network vs non-social co-location network

```{r social_network_non_social, echo=FALSE}
...
p_alters <- ggplot(df_stats , aes(x=Rank, y=mean_Pi_alters_ratio, 
                      # shape=category,
                      color=category)) + 
  geom_point(size=3) +
  theme(
  ...
  scale_y_continuous(labels = scales::percent) + 
  geom_errorbar(aes(ymin=lower_Pi_alters_ratio, ymax=upper_Pi_alters_ratio), 
                width=.2,
                position=position_dodge(0.05)) + 
  facet_wrap(~dataset, scales = "free") + 
  labs(x = "Included number of alters", 
...
ggsave(
  filename = "extra_social_vs_non_social.pdf",
  device = "pdf",
  width = 9, height = 3.2,
  path = "fig/"
)
